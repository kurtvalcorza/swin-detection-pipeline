...
The v1 inference contract excludes instance-mask output even though Mask R-CNN internally produces masks; only object-detection boxes, class labels and scores are exposed. It does not provide tracking, person re-identification, keypoints, oriented boxes, panoptic segmentation or semantic segmentation. The pretrained runtime is not a fine-tuner and cannot turn a private class taxonomy into a trained DIMER artifact. The current repository does not provide a fine-tuning path, production inference service, calibration workflow or deployment acceptance threshold. Images requiring non-RGB bands, specialized medical interpretation, tiny-object aerial detection at resolutions far outside the COCO regime, or safety-critical real-time behavior are outside the qualified scope. Use for autonomous consequential decisions, surveillance/profiling, weapons targeting, unlawful discrimination, or deceptive evidence generation is unacceptable. Licensing no longer blocks hosting of the adaptation-lineage checkpoint, but adaptation remains out of scope in this release.
...
Key risks include false positives, missed detections, poor localization, class confusion and silent distribution shift; these can waste resources or mislead users who treat detections as verified observations. Uncalibrated scores can encourage unjustified confidence. COCO-derived features can inherit dataset skews, particularly in scenes involving people and geographically specific contexts. The `.pth` trust boundary remains inherently code-capable even though the exact pinned bytes are verified first. The previous licensing uncertainty for DIMER redistribution of the adaptation-lineage checkpoint is resolved by the MIT license in the upstream `SwinTransformer/storage` repository; redistribution must preserve the applicable MIT copyright and permission notice. The existence of a convenient pretrained API can also lower the barrier to repurposing for surveillance, profiling or other harmful applications. The strongest control is therefore a combination of technical integrity, explicit capability boundaries, human review and use-policy governance. Future fine-tuning could amplify any of these harms if operator data or labels are biased, poorly governed or mismatched to deployment conditions.
