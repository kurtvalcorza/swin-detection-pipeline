...
# Swin-T + Mask R-CNN (org.valcorza.swin-detection 0.1.0) — Object Detection (Pretrained Inference; Adaptation Scaffold)
...
This pipeline provides a ready-to-run interactive Google Colab notebook for the release-grade capability, pretrained inference through the public `DimerSwinDetector` API. The gradient-adaptation scaffold is exercised only by `swin_detection_scaffold_smoke_colab.ipynb`, a `SMOKE`-profile notebook that is not a release-grade tutorial and is not listed here:
