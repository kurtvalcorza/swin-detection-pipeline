...
EXPECTED_MODEL_ID = "open-mmlab/mmdetection:mask-rcnn_swin-t-p4-w7_fpn_1x_coco"
...
# The card's front-matter `base_model` (MODEL_CARD_SPEC 1.1 header block, not edited by the notebook work) names
# the frozen Microsoft/SwinTransformer adaptation lineage, not the OpenMMLab inference checkpoint MODEL_ID pins.
CARD_BASE_MODEL = "SwinTransformer/storage mask_rcnn_swin_tiny_patch4_window7_1x.pth (release v1.0.3, asset 36780486)"
...
EXPECTED_OUTPUTS = (
    ...
    "outputs/swin_detection_task_inference_evaluation_report.json",
    "outputs/swin_detection_task_inference_result.json",
    "outputs/swin_detection_task_inference_detections.csv",
)
...
CODE_MARKERS = (
    ...
    "boxes_from_yolo_labels(label_text, width, height)",
...
MARKDOWN_MARKERS = (
    "**Capability:** pretrained COCO-80 object detection",
...
def validate_model_card() -> None:
    ...
    _check(f"base_model: {CARD_BASE_MODEL}" in front, f"MODEL_CARD.md base_model must be {CARD_BASE_MODEL!r}")
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    191_487_694: "SwinTransformer/storage v1.0.3 asset mask_rcnn_swin_tiny_patch4_window7_1x.pth",
}
EXTERNAL_WEIGHT_DIGESTS: dict[str, str] = {
    "b67f9d6cd62a4d723c78faec1b49cbf548faa22437264defb00f2f6e54d21b78": "SwinTransformer/storage v1.0.3 mask_rcnn_swin_tiny_patch4_window7_1x.pth (not staged)",
}
...
def _validate_notebook_content(
    ...
    host = _load_tool("notebook_template").TEMPLATE.get("model_host", {})
    _check(f"- **External access:** {host.get('name', 'the Hugging Face Hub')} only" in markdown, f"{path.name}: the External-access bullet must name the model host only")
    ref = host.get("reference_url", f"https://huggingface.co/{model_id}")
